...
It’s quite annoying that anything we pass in also needs to be passed back if
 we want to use it again, in addition to any data resulting from the body of the function that we might want to return as well.
...
Luckily for us, Rust has a feature for this concept, and it’s called references.
*/
